    ...
    def _action_done(self):
        ...
        for picking in self:
            if picking.group_id and picking.group_id.sale_id:
                if picking.group_id.sale_id.state in ['damaged_item', 'item_returned']:
                    continue

                pickings_pick = picking.group_id.sale_id.picking_ids.filtered(
                    lambda x: x.picking_type_code == 'internal' and x.state not in ['cancel'])
                pickings_out = picking.group_id.sale_id.picking_ids.filtered(
                    lambda x: x.picking_type_code == 'outgoing' and x.state not in ['cancel'])
                if all(picking.state == 'done' for picking in
                       pickings_out) and picking.group_id.sale_id.state != 'shipped':
                    picking.group_id.sale_id.sudo().write({
                        'state': 'shipped'
                    })
                    continue
                if all(picking.state == 'done' for picking in pickings_pick) and picking.group_id.sale_id.state not in [
    ...
    def filter_on_product(self, barcode):
        ...
        if not product:
            ...
            if bc and bc[0]['product_id']:
                product = self.env['product.product'].search_read([('id', '=', bc[0]['product_id'][0])], ['id'], limit=1)
